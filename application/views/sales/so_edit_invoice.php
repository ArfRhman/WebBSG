                    ...
                    <form class="form-horizontal" enctype="multipart/form-data" action="<?php echo site_url('sales/so/update_invoice/'.$this->uri->segment(4));?>" method="post">
                        <fieldset>
                            <div class="form-group">
                                ...
                                <div class="col-md-3">
                                <input id="name" name="do_no" placeholder="Invoice No" class="form-control" type="text" value="<?php echo $so->row()->do_no ?>"></div>                                                                                     <label class="col-md-2 control-label" for="email">Item ID</label>                                          <div class="col-md-3">                                                <select name="item" class="form-control" id="itemID">
                                    <?php
                                    $sql = $this->mddata->getAllDataTbl('tbl_dm_item');
                                    foreach($sql->result() as $s)
                                    {
                                        ?>
                                        <option value="<?php echo $s->id; ?>" <?php echo  ($s->id == $so->row()->item_id)?'selected' : '' ?>><?php echo $s->code ?> - <?php echo $s->nama ?></option>
                                        <?php
                                    }
                                    ?>
                                </select></div>
